...
public class GlassBottleWithSugar extends Block {
    public GlassBottleWithSugar(Settings settings) {
        super(settings);
    }
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!stack.isEmpty()){
            if(stack.getItem() == ModItems.SPARKLING_WATER){
                stack.decrement(1);
                world.setBlockState(pos, ModBlocks.GLASS_BOTTLE_WITH_SUGAR_AND_SPARKLING_WATER.getDefaultState());
